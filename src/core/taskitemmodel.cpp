...
QVariant TaskItemModel::data(const QModelIndex &index, int role) const
{
    ...
    if (role == Qt::DecorationRole) {
        if (index.column() == 0) {
            if (isActiveTask) {
                if (m_tomato->isActiveTask(task->id())) {
                    if (m_tomato->state() == Tomato::Working
                            || m_tomato->state() == Tomato::OverWorking) {
                        return theme::icon(theme::IconTaskViewTaskWorking);
                    }
...
QModelIndex TaskItemModel::index(int row, int column, const QModelIndex &parent) const
{
    ...
    return createIndex(row, column, parentTask->children().at(row));
}
                                 ...
                                 const QModelIndex &parent)
{
    ...
    if (data->hasFormat(TaskIdsMimeData)) {
        ...
        foreach (int id, ids) {
            m_tomato->moveTask(id, parentItem->id(), row);
        }
...
void TaskItemModel::tomato_aboutToBeTaskInserted(Task *parent,
                                                 int first,
                                                 int last)
{
...
void TaskItemModel::tomato_taskInserted(Task */*parent*/,
                                        int /*first*/,
                                        int /*last*/)
{
    endInsertRows();
...
void TaskItemModel::tomato_aboutToBeTaskRemoved(Task *parent,
                                                int first,
                                                int last)
{
    if (parent == m_tomato->rootTask())
...
void TaskItemModel::tomato_taskRemoved(Task */*parent*/,
                                       int /*first*/,
                                       int /*last*/)
{
    endRemoveRows();
...
void TaskItemModel::tomato_aboutToBeTaskDataChanged(Task */*parent*/,
                                                    int /*first*/,
                                                    int /*last*/)
{
}
...
{
    Task *firstTask = parent->children().at(first);
    Task *lastTask = parent->children().at(last);
